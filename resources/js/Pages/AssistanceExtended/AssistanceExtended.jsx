...
export default function AssistanceExtended({ assistances: initialAssistances, disabled = false }) {
    ...
    // Manage assistances state locally
    const [assistances, setAssistances] = useState(
        initialAssistances && initialAssistances.length > 0
            ? initialAssistances
            : [
                {
                    id: `new-${Date.now()}`,
    ...
    const {
        paginatedData: paginatedAssistances,
        searchTerm,
        setSearchTerm,
        currentPage,
        setCurrentPage,
    ...
    const {
        ...
        queryKey: ["assistance-modifications"],
        queryFn: async () => {
            const { data } = await axios.get(`${APP_URL}/modifications/assistance-extended`);
            return data;
        },
    ...
    const handleInputChange = useCallback((rowId, event) => {
        ...
        const updatedAssistances = assistances.map(assist => 
            assist.id === rowId 
                ? { ...assist, [name]: value }
                : assist
        );
    ...
    const hasChanges = useMemo(() => {
        ...
        return JSON.stringify(originalData) !== JSON.stringify(assistances);
    }, [originalData, assistances]);
    
    // Validate assistances before submission
    const validateAssistances = useCallback(() => {
        const errors = [];
        
        assistances.forEach((assist, index) => {
            // At least one field should be filled
            const hasData = assist.agency_officials_groups || assist.type_kind_of_assistance || 
                           assist.amount || assist.beneficiaries;
            if (!hasData) {
                errors.push(`Row ${index + 1}: At least one field is required`);
            }
            
            // Validate amount if provided
            if (assist.amount && (isNaN(assist.amount) || parseFloat(assist.amount) < 0)) {
    ...
    const handleSubmit = async () => {
        ...
        const validationErrors = validateAssistances();
        ...
        try {
            ...
            const cleanedAssistances = assistances.map(assist => ({
                ...assist,
                id: typeof assist.id === 'string' ? null : assist.id
            }));
            
            const response = await axios.post(`${APP_URL}/assistance-extendeds`, {
                assistances: cleanedAssistances,
            });
            ...
            const errorMessage = err.response?.data?.message || 
                                ...
                                "Failed to save assistance records. Please try again.";
                ...
                <div className="md:overflow-x-auto md:rounded-lg md:border md:border-slate-200">
                    <table className="w-full text-sm">
                        ...
                        <tbody className="flex flex-col md:table-row-group gap-4 md:gap-0">
                            {paginatedAssistances.length === 0 && searchTerm ? (
                                <tr>
                                    <td colSpan="4" className="p-8 text-center">
                                        <div className="flex flex-col items-center justify-center space-y-3">
                                            ...
                                            <p className="text-sm text-slate-500">
                                                No records match "<strong>{searchTerm}</strong>"
                                            </p>
                                            <button
                            ...
                            ) : paginatedAssistances.map((row, index) => {
                                const fields = [
                                    "agency_officials_groups",
                                    "type_kind_of_assistance",
                                    "amount",
