...
import { Droplets, Loader2, History, Clock, Cloud, Eye, Edit, ChevronDown, ChevronUp } from "lucide-react";
...
export default function WaterServiceHistoryIndex() {
    ...
    const { data: historyData, isLoading } = useQuery({
        queryKey: ["water-service-history"],
        queryFn: async () => {
            const { data } = await axios.get(`${APP_URL}/api/water-service-history`);
            return data;
        },
        staleTime: 1000 * 60 * 5,
    });
    ...
    return (
        <SidebarProvider>
            <AppSidebar />
            <Head>
                <title>Water Service Report History</title>
                        ...
                        <Card>
                            <CardContent className="pt-6">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h1 className="font-semibold text-slate-800 text-xl">Water Service Report History</h1>
                                        <p className="text-sm text-slate-600">
                                            View all your water service reports organized by typhoon
                                        </p>
                                    </div>
                                </div>
                            </CardContent>
                                                        ...
                                                        {typhoonGroup.reports.map((report, index) => (
                                                            ...
                                                            >
                                                                ...
                                                                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                                                    ...
                                                                    {report.source_of_water && (
                                                                        <div>
                                                                            <p className="text-xs font-medium text-gray-600 mb-1">Source of Water</p>
                                                                            <p className="text-sm text-gray-700">{report.source_of_water}</p>
                                                                        </div>
                                                                    )}
                                                                    {report.barangays_served && (
                                                                        <div>
                                                                            <p className="text-xs font-medium text-gray-600 mb-1">Barangays Served</p>
                                                                            <p className="text-sm text-gray-700">{report.barangays_served}</p>
                                                                        </div>
                                                                    )}
                                                                </div>
                                                            </div>
                            ...
                            <Card>
                                <CardContent className="py-16 text-center">
                                    ...
                                    <p className="text-sm text-gray-500">
                                        Your water service reports will appear here
                                    </p>
                                </CardContent>
                            </Card>
                        )}
                ...
                <DialogContent className="max-w-2xl">
                    ...
                    {selectedReport && (
                        <div className="space-y-4">
                            ...
                            {selectedReport.source_of_water && (
                                <div>
                                    <p className="text-sm font-medium text-gray-600 mb-2">Source of Water</p>
                            ...
                            {selectedReport.barangays_served && (
                                <div>
                                    <p className="text-sm font-medium text-gray-600 mb-2">Barangays Served</p>
                                    <p className="text-sm text-gray-700 bg-gray-50 p-3 rounded-lg">
                                        {selectedReport.barangays_served}
                                    </p>
                                </div>
